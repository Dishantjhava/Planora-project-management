...
const Dashboard = ({ onLogout }) => {
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (e.key === 'Escape') {
        ...
        setShowInviteModal(false); setShowTeamModal(false); setShowAddTask(false);
      }
    };
  ...
  // ══════════════════════════════════════════════════════════════════
  // KEY FIX: Derive real task stats from the tasks array
  // ══════════════════════════════════════════════════════════════════
  const getProjectTaskStats = (projectId) => {
    const pTasks = tasks.filter(t => t.projectId === projectId);
    const done   = pTasks.filter(t => t.status === 'done').length;
    return { total: pTasks.length, done };
  };

  // Overall progress = done tasks / total tasks across ALL tasks
  const { totalTasks, completedTasks, overallProgress } = useMemo(() => {
    // combine project base tasks + manual tasks
    const projectBaseTotals = projects.reduce((s, p) => s + p.tasks, 0);
    const projectBaseDone   = projects.reduce((s, p) => s + p.completedTasks, 0);

    // add tasks created via task modal
    const manualTasks = tasks.length;
    const manualDone  = tasks.filter(t => t.status === 'done').length;

    const total     = projectBaseTotals + manualTasks;
    const completed = projectBaseDone   + manualDone;
    const pct       = total > 0 ? Math.round((completed / total) * 100) : 0;

    return { totalTasks: total, completedTasks: completed, overallProgress: pct };
  }, [projects, tasks]);
  ...
  const handleSendInvite = () => {
    ...
    const invite = {
      ...
      role: inviteForm.role, avatar: getInitials(inviteForm.name),
      color, status: 'pending', sentAt: new Date().toLocaleString(),
    };
        ...
        <header className="header">
          ...
          <div className="header-right">
            ...
            <button className="add-task-btn" onClick={() => openAddTaskModal()} title="Add Task (Ctrl+T)">
              <svg viewBox="0 0 24 24" fill="none" width="16" height="16"><path d="M9 11L12 14L22 4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M21 12V19C21 19.53 20.79 20.04 20.41 20.41C20.04 20.79 19.53 21 19 21H5C4.47 21 3.96 20.79 3.59 20.41C3.21 20.04 3 19.53 3 19V5C3 4.47 3.21 3.96 3.59 3.59C3.96 3.21 4.47 3 5 3H16" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
              <span>Add Task</span>
          ...
          <div className="bento-grid">
            ...
            {stats.map((stat, i) => (
              <div key={i} className="bento-stat">
                <div className="bento-stat-icon" style={{ color: stat.color, background: `${stat.color}18` }}>{stat.icon}</div>
            ...
            {/* ── Projects card — spans 2 rows, 2 cols (main large card) ── */}
                      ...
                      <div key={project.id} className="project-item" onClick={() => setSelectedProject(project)}>
                        <div className="project-header">
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            <button className="btn-task-inline" title="Add task to this project"
                        ...
                        <div className="project-meta">
                          ...
                          <span className="project-tasks">
                            ...
                            {pTasks.length > 0 && <span className="extra-tasks-badge">+{pTasks.length} manual</span>}
                          </span>
                        ...
                        {/* Tasks list with clickable status dots */}
                        {pTasks.length > 0 && (
                          <div className="project-tasks-preview" onClick={e => e.stopPropagation()}>
                            <div className="tasks-preview-header">
                              Tasks ({pTasks.length}) · {pDone} done
                            </div>
                              ...
                              {pTasks.slice(0, 3).map(task => {
                                ...
                                return (
                                  <div key={task.id} className="task-preview-item">
                                    {/* CLICKABLE STATUS DOT — cycles status */}
                                    <button
                                      className={`task-status-dot task-status-${task.status} task-status-btn`}
                                      onClick={(e) => toggleTaskStatus(task.id, e)}
                                      title={`Status: ${STATUS_LABELS[task.status]} — click to advance`}
                                    />
                                    <span className={`task-preview-title ${task.status === 'done' ? 'task-done-text' : ''}`}>
                                      {task.title}
                                    </span>
                                    <div className="task-preview-meta">
                                      ...
                                      <button className="task-delete-btn" onClick={e => handleDeleteTask(task.id, e)} title="Delete">✕</button>
                                    </div>
                                  </div>
                                );
                              })}
                              {pTasks.length > 3 && (
                                <div className="tasks-preview-more">+{pTasks.length - 3} more tasks</div>
                              )}
                            </div>
                          </div>
                        )}
            ...
            {/* ── Team Members ── */}
            <div className="bento-team card bento-card-inner">
              <div className="bento-section-title-row">
                <span className="bento-section-title">Team <span className="team-count-badge">{teamMembers.length}</span></span>
                <div style={{display:'flex',gap:'.4rem'}}>
                  <button className="btn-link" onClick={() => setShowTeamModal(true)}>All</button>
                  <button className="btn-invite-small" onClick={openInviteModal}>+ Invite</button>
                </div>
              </div>
              <div className="bento-team-list">
                {teamMembers.slice(0, 5).map((m, i) => (
                  <div key={i} className="bento-team-row">
                    <div className="member-avatar" style={{ backgroundColor: m.color }}>
                      {m.avatar}<span className={`status-indicator ${m.status}`}></span>
                    </div>
                ...
                {teamMembers.length > 5 && (
                  <button className="show-more-btn" onClick={() => setShowTeamModal(true)}>+{teamMembers.length - 5} more</button>
                )}
                {pendingInvites.length > 0 && (
                  <div className="pending-preview">
                    <span className="pending-label">⏳ {pendingInvites.length} pending</span>
                ...
                {upcomingDeadlines.length > 0 ? upcomingDeadlines.map((item, i) => (
                  <div key={i} className="deadline-item">
                    ...
                    <div className="deadline-info">
                      ...
                      <div className="deadline-days">{item.days === 1 ? 'Tomorrow' : `${item.days}d left`}</div>
                    </div>
          ...
          <div className="modal task-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-group">
                ...
                <input type="text" placeholder="e.g. Design login screen mockups"
                  value={taskForm.title} onChange={e => setTaskForm({ ...taskForm, title: e.target.value })}
                  className={taskFormErrors.title ? 'error' : ''} autoFocus/>
                {taskFormErrors.title && <span className="error-text">{taskFormErrors.title}</span>}
              </div>
              <div className="form-group">
                <label>Description</label>
                <textarea placeholder="Describe what needs to be done…" rows="3"
                  ...
                  <div className="priority-selector">
                    {[['high','🔴 High'], ['medium','🟡 Medium'], ['low','🟢 Low']].map(([val, label]) => (
                      <button key={val} type="button"
                        ...
                        onClick={() => setTaskForm({ ...taskForm, priority: val })}>{label}</button>
                    ))}
                  </div>
                </div>
                <div className="form-group">
                ...
                <div className="task-preview-card">
                  ...
                  <div className="task-preview-inner">
                    ...
                    <div className="task-preview-details">
                      {taskForm.projectId && (
                        <span className="task-preview-chip">📁 {projects.find(p => p.id.toString() === taskForm.projectId)?.name}</span>
                      )}
                      {taskForm.assigneeId && (
                        <span className="task-preview-chip">👤 {teamMembers.find(m => m.id.toString() === taskForm.assigneeId)?.name}</span>
                      )}
                      {taskForm.dueDate && (
          ...
          <div className="modal modal-large team-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body" style={{ padding: 0 }}>
              <div className="team-section">
                <div className="team-section-header"><h4>Active Members</h4><span className="team-section-count">{teamMembers.length}</span></div>
                <div className="team-members-full">
                  {teamMembers.map(member => (
                    <div key={member.id} className="team-member-row">
                      <div className="member-avatar large" style={{ backgroundColor: member.color }}>
                        {member.avatar}<span className={`status-indicator ${member.status}`}></span>
                      </div>
                  ...
                  <div className="team-members-full">
                    {pendingInvites.map(invite => (
                      <div key={invite.id} className="team-member-row pending-row">
                        <div className="member-avatar large" style={{ backgroundColor: invite.color, opacity: 0.7 }}>{invite.avatar}</div>
                        <div className="member-info-full">
                      ...
                      return (
                        <div key={task.id} className="detail-task-item">
                          ...
                          <div className="detail-task-info">
                            <span className={`detail-task-title ${task.status === 'done' ? 'task-done-text' : ''}`}>{task.title}</span>
